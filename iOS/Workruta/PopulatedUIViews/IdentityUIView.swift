...
struct IdentityUIView: View {
    
    let this: IdentityViewController
    ...
    var body: some View {
            ...
            ScrollView{
                VStack(spacing: 20) {
                    VStack(alignment: .leading) {
                        Text("Identity").foregroundColor(Colors.mainColor).font(.system(size: 17)).padding(EdgeInsets(top: 0, leading: 20, bottom: 0, trailing: 0))
                        Picker("", selection: $idIndex){
                            ForEach(0..<Constants.validIDs.count, id: \.self){ index in
                                Text(Constants.validIDs[index])
                            }
                        }
                        .customPickerStyle(index: $idIndex, items: Constants.validIDs, font: .system(size: 17), padding: 10)
                        .frame(width: UIScreen.main.bounds.width - 50)
                        .overlay(RoundedRectangle(cornerRadius: 10.0).stroke(Colors.mainColor, lineWidth: 1.0))
                        .disabled(requesting)
                    }
                    TextField("", text: $number)
                        .modifier(PlaceholderStyle(show: number.isEmpty, text: "ID Number"))
                        .padding(10)
                    ...
                    SecureField("", text: $password)
                        .modifier(PlaceholderStyle(show: password.isEmpty, text: Strings.password))
                        .padding(10)
                        .background(Colors.white)
                        .cornerRadius(10)
                        .foregroundColor(Colors.black)
        ...
        .overlay(
            HStack(alignment: .center, spacing: 10){
                (Text(Image(systemName: "chevron.left")) + Text("Identity Information"))
    ...
    func saveData() {
        ...
        if idIndex == 0 || number == "" || isDate == "" || exDate == "" || password == "" {
        ...
        if !available && uiImage.cgImage == nil && uiImage.ciImage == nil {
            this.showAlertBox(title: "", msg: "Please take a picture of your ID", btnText: "Close")
            self.requesting = false
            return
        }
        if idIndex == iIdIndex && number == iNumber && isDate == iIsDate && exDate == iExDate && !imageChanged {
            this.showAlertBox(title: "", msg: "No changes made", btnText: "Close")
            self.requesting = false
            return
        }
        guard let url = URL(string: Constants.submitIDUrl) else {
            print("URL not found")
            return
        }
        self.progess = 0
    ...
    func getParameters() -> [String: String] {
        ...
        var parameter: [String: String] = [
            ...
            "password": password,
            "number": number,
            "type": Constants.validIDs[idIndex],
